...
public abstract class ActionSpell(
    ...
    public override final fun perform(invoker: Character, receiver: Any): Boolean {
...
public class SpellSleep : ActionSpell("Sleep") {
    override fun apply(invoker: Character, receiver: Character): Unit {
       // Generate a random number (sleepChance). When sleepChance is greater or equal to receiver.sleepResistance the spell is successful.
       val sleepChance: Int = if (receiver.category == Actor.Category.Player) Random.nextInt(0, 2) else Random.nextInt(0, 16)
       val sleepResistance: Int = receiver.sleepResistance
       receiver.isAsleep = sleepChance >= sleepResistance
    }
}
